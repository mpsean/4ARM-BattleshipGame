...
const WelcomeScreen = ({ startPlay }) => {
  ...
  return (
    <div class="bg-front bg-cover dark:bg-black/20 dark:bg-blend-darken h-screen w-screen bg-no-repeat bg-center py-16 px-24 flex flex-col items-center justify-center">
      <img src={logoImg} width={300} className="pb-8" />
      <h1 className="font-montserrat text-3xl font-black text-white drop-shadow-xl">
        Rules
      </h1>
      <div className="flex flex-col px-24 py-12 bg-gray-400/50 dark:bg-white/20  rounded-3xl max-w-screen-lg m-3 lg:m-6">
        <div className="flex justify-center">
          <h2 className="font-museo text-white font-bold text-2xl text-center">
            Welcome {userId}.
          </h2>
        </div>
        ...
        <div className="flex justify-center">
          <h2 className="font-museo text-white font-bold text-xl">
            {opponentMessage}
          </h2>
        </div>
      </div>
      <button
        className="flex justify-center items-center gap-2 px-7 py-4 font-montserrat font-bold text-xl leading-none ring-4 ring-white text-white rounded-full bg-sky-700 dark:bg-sky-950  hover:bg-green-800"
        onClick={startGame}
      >
        Play
      </button>
      <div>
        {/* Conditionally render the video based on whether it has ended */}
      {!videoEnded && (
        <div>
          <video 
            src="/assets/vids/lore-vid.mp4"
            className="absolute px-40 inset-0 h-screen w-fit object-cover"
            autoPlay
            muted
            onEnded={handleVideoEnd} // Set state when video ends
          />
        </div>
      )}
      </div>
